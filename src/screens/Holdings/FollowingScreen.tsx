...
const FollowingScreen = () => {
  ...
  const [followingList, setFollowingList] = useState<UserInfoData[]>([]);
  ...
  const fetchFollowingList = async (start = 0, isRefresh = false) => {
    try {
      if (isRefresh) {
        setRefreshing(true);
      } else if (start === 0) {
      ...
      const result = await userService.fetchFollowingsList(start, 20);

      if (isRefresh || start === 0) {
        setFollowingList(result.list);
      } else {
        setFollowingList(prev => [...prev, ...result.list]);
      }
      ...
      // 设置关注状态
      const followingUserIds = result.list.map(user => user.id);
  ...
  const handleRefresh = () => {
    fetchFollowingList(0, true);
  };

  const handleLoadMore = () => {
    if (hasMore && !loading && !refreshing) {
      fetchFollowingList(nextStart);
    }
  };
  ...
  const renderFooter = () => {
    if (!hasMore || followingList.length === 0) return null;
    return (
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>{i18n.t('holdings.followingPage.empty')}</Text>
      <Text style={styles.emptySubtext}>{i18n.t('holdings.followingPage.emptySubtext')}</Text>
    </View>
  );

  return (
    <SafeAreaView style={styles.container}>
      <AppBar title={i18n.t('holdings.followingPage.title')} />
      ...
      <FlatList
        ...
        contentContainerStyle={followingList.length === 0 ? styles.emptyContentContainer : styles.contentContainer}
      />
    </SafeAreaView>
  );
};
